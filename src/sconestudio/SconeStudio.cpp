...
void SconeStudio::clearScenario()
{
	...
	if ( scenario_ )
		for ( const auto& p : scenario_->GetExternalFiles().GetVec() )
...
bool SconeStudio::createScenario( const QString& any_file )
{
	...
	try {
		...
		if ( scenario_->HasModel() )
		{
			...
			bool use_static_camera = scenario_->GetModel().HasRootBody() && scenario_->GetModel().GetRootBody().IsStatic();
	...
	for ( auto& filename : reloadFiles ) {
		for ( auto* e : codeEditors )
			if ( e->fileName == filename )
				reload |= e->reload();
		if ( scenario_ ) {
			if ( scenario_->GetExternalFiles().Contains( filename.toStdString() ) )
				reload = true;
		}
	}
	reloadFiles.clear();
